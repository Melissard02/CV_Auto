...
def get_inputs():
    ...
    while True:
        # example = input("Ask for input: ")
        company = input("Company Name: ")
        job_title = input("Job Title: ")
        
        confirm = input(f"\nIs this correct?\nCompany: {company}\nJob Title: {job_title}\n(y/n)\n> ")
        if confirm.lower() == "y":
            # IF MORE PLACEHOLDERS WORDS ARE USED, INCLUDE THEM IN THIS APPEND STATEMENT
...
def generate_cover_letters(jobs):
    ...
    # INCLUDE ANY ADDITIONAL PLACEHOLDERS IN THE FOR STATEMENT
    for company, job_title in jobs:
        print("Looking for template at:", os.path.abspath("cover_letter_template.docx"))
        print("Exists?", os.path.exists("cover_letter_template.docx"))

        doc = Document("cover_letter_template.docx")

        # PLACE HERE THE PLACEHOLDER TEXT USED IN THE ACTUAL WORD DOCUMENT
        # THE TEXT MUST MATCH EXACTLY WHAT IS IN THE QUOTATIONS 
        replacements = {
        ...
        # THE FILENAME IS A UNIQUE COMPOSITE KEY, ADD ADDITIONAL VARIABLES IF NEEDED 
